...
$('#title_head').text('Evaluated Travel Pass')
$('#title_bread').text('Evaluated Travel Pass')

socket.on('connect', () => {
    const municipal_role = $('#municipality').val()
    if (municipal_role) {
        socket.emit('getEvaluatedRecords', municipal_role);
    } else {
        let def_val = "all";
        socket.emit('getEvaluatedRecords', def_val);
    }
    $('#municipality').change(function () {
        const municipality_admin = $('#municipality').val()
        socket.emit('getEvaluatedRecords', municipality_admin);
        console.log(municipality_admin)
    })
    // socket.emit('getEvaluatedRecords');
});
//reload DT
socket.on('onReloadPassedTravel', () => {
    socket.emit('getEvaluatedRecords')
});
socket.on('collectEvaluatedRecords', data => {
    console.log(data)
    const dataSet = [];
