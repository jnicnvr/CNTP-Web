...
$('#title_head').text('Travel Pass')
$('#title_bread').text('Travel Pass')

socket.on('connect', () => {
    
    const municipal_role = $('#municipality').val()
    if (municipal_role) {
        socket.emit('getTravelPassRecords', municipal_role);
    } else {
        let def_val = "all";
        socket.emit('getTravelPassRecords', def_val);
    }
    $('#municipality').change(function () {
        const municipality_admin = $('#municipality').val()
        socket.emit('getTravelPassRecords', municipality_admin);
...
socket.on('onReloadTravelPass', () => {
    console.log("onReloadTravelPass")
    socket.emit('getTravelPassRecords');
});
socket.on('collectTravelPassRecords', data => {
    console.log(data)
    const dataSet = [];
